...
% Extract voltage magnitudes and phase angles
V = x2V(x, Vg, Ng, N);
phi = x2phi(x, Ng, N);

%% 1. Voltage phasor at Bus 4
V4 = V(4) * exp(1j * phi(4));
V4_rms = abs(V4);                               % Item 1: Voltage RMS at Bus 4 in pu
V4_angle = phi(4);                              % Item 2: Voltage phase angle at Bus 4 in radians
fprintf('1. Voltage phasor at Bus 4: %.5f ∠ %.5f° (pu)\n', V4_rms, rad2deg(V4_angle));

%% 2. Real and reactive power generation at Bus 2
Pgen2 = 300;                                    % Item 3: Real power generation at Bus 2 in MW
...
Pload3 = 170; % Real power load at Bus 3 (MW)
Pload4 = 200; % Real power load at Bus 4 (MW)
Pload_total = Pload2 + Pload3 + Pload4; % Total system load (MW)

Pgen_total = Pgen2 + (Pload_total - sum(Pknown) * 100); % Total generation in MW
...
% Calculate power flow from Bus 1 to Bus 3
I13 = (V1 - V3) * Ybus(1, 3); % Current flowing from Bus 1 to Bus 3
...
sol = [ V4_rms;       % Item 1: Voltage RMS at Bus 4 in pu
        ...
        Pgen_total;   % Item 5: Total real power generation in MW
